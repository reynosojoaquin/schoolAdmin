...
namespace SchoolControl.Server.Controllers
{
    ...
    public class CalificacionesController : ControllerBase
    {
        ...
        public async Task<IActionResult> Editar(List<CalificacionesDTO> calificaciones)
        {
            ...
            try
            {
                /* var registros = _dbContext.Database
                     .SqlQueryRaw<int>("SELECT public.asignarcalificacionesestudiantes(@p_estudianteid,@p_cursoid) as resultado",
                 new NpgsqlParameter("p_estudianteid", NpgsqlDbType.Integer)
                 {Value = calificaciones[0].Estudianteid },
                 new NpgsqlParameter("p_cursoid", NpgsqlDbType.Integer) { 
                 Value =calificaciones[0].cursoID }).FirstOrDefault();*/
                await _dbContext.Database.OpenConnectionAsync();
                DbCommand command = _dbContext.Database.GetDbConnection().CreateCommand();
                command.CommandText = "SELECT public.asignarcalificacionesestudiantes(@p_estudianteid,@p_cursoid) as resultado";
                command.Parameters.Add(new NpgsqlParameter("p_estudianteid", calificaciones[0].Estudianteid));
                command.Parameters.Add(new NpgsqlParameter("p_cursoid", calificaciones[0].cursoID));
                var result = (int)await command.ExecuteScalarAsync();
                // Cierra la conexión después de ejecutar el comando
                await _dbContext.Database.CloseConnectionAsync();

                // Recargar datos después de la ejecución para reflejar cambios en EF Core
                await _dbContext.Entry(DBCalificaciones).ReloadAsync();
                foreach (var cal in calificaciones)
                    {
        ...
        public async Task<IActionResult> GetDataCalificaciones([FromQuery] int cursoID, [FromQuery] int estudianteID, [FromQuery] int asigID)
        {
            ...
            try
            {
                ...
                calificacionesLst = (from calificaciones in _dbContext.Calificaciones
                                     ...
                                     select new InfoCalificacionesDTO
                                     {
                                         ...
                                         estudianteID = calificaciones.Estudianteid

                                     }
                                      ).ToList().Where(c => c.estudianteID == estudianteID & c.cur_id == cursoID & c.asig_id == asigID).ToList();
